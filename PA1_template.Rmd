...
```{r fillinmissingvalues, echo=TRUE, results='asis', cache=FALSE}
...
```{r missingvaluesagain, echo=TRUE, results='asis', cache=FALSE}
na_values_fillin <- sum(is.na(movementData_fillin$steps))
```

The number of missing values is now `r na_values_fillin`.

Based on this new dataset a new histogram is created and new values for mean and median are calculated:

```{r stepsperdayagain, echo=TRUE, results='asis', cache=FALSE}
...
```{r weekdaysvsweekends, echo=TRUE, results='asis', cache=FALSE}
